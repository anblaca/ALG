...
class SAR_Project:
    ...
    def __init__(self):
        ...
        self.use_spellsuggester = False #valor por defecto
    ...
    def set_spellsuggester(self, v):
        """
        Cambia el modo de spellsuggest por defecto.
        input: "v" booleano.
        UTIL PARA LA VERSION CON spellsuggest
        si self.use_spellsuggester es True las consultas se resolveran aplicando stemming por defecto.
        """
        self.use_spellsuggester = v
    ...
    def index_dir(self, root, **args):
        ...
        self.spellsuggest = args['spellsuggest']
        ...
        if self.spellsuggest:
            with open('{}_voc'.format(args['index']),'w') as tmp_file:
                for term in self.index.keys():
                    tmp_file.write("{}\n".format(term))
            self.spell = sg.TrieSpellSuggester('{}_voc'.format(args['index']))
    ...
    def get_posting(self, term, field='article'):
        ...
        if self.use_spellsuggester and arrayIdNews == []:
            suggestion = self.spell.suggest(term,"levenshtein",1)
            for sugg_term in suggestion.keys():
                arrayIdNews = self.or_posting(arrayIdNews,self.get_posting(sugg_term))
        return arrayIdNews
